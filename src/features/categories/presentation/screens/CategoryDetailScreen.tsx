...
export default function CategoryDetailScreen() {
    ...
    const courseStudentIdsParam: string[] | undefined =
        route.params?.courseStudentIds
    const di = useDI()
    const theme = useTheme()
    const getGroupsUC = di.resolve<GetGroupsByCategoryUseCase_v2>(
        TOKENS.GetGroupsByCategoryUC_v2
    )
    const createGroupUC = di.resolve<CreateGroupUseCase_v2>(
        TOKENS.CreateGroupUC_v2
    )
    const updateGroupUC = di.resolve<UpdateGroupUseCase_v2>(
        TOKENS.UpdateGroupUC_v2
    )
    const deleteGroupUC = di.resolve<DeleteGroupUseCase_v2>(
        TOKENS.DeleteGroupUC_v2
    )
    const addStudentUC = di.resolve<AddStudentToGroupUseCase_v2>(
        TOKENS.AddStudentToGroupUC_v2
    )
    const removeStudentUC = di.resolve<RemoveStudentFromGroupUseCase_v2>(
        TOKENS.RemoveStudentFromGroupUC_v2
    )
    ...
    const getAvailableStudentsForCategory = (): string[] => {
        const courseStudents = Array.isArray(courseStudentIdsParam)
            ? courseStudentIdsParam
            : []
                ...
                >
                    ...
                    >
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            >
                                {item.studentIds.length} estudiantes
                            </Text>
                        </View>
                    </View>
                    {isTeacher && (
                        ...
                        >
                            ...
                            <IconButton
                                ...
                                onPress={async () => {
                                    if (!item._id) return
                                    await deleteGroupUC.execute(item._id)
                                    await refresh()
                                }}
                            />
                        </View>
                    ...
                    {item.studentIds.map((sid) => (
                        <Chip
                            ...
                            avatar={
                                <Avatar.Text
                                    size={24}
                                    label={sid.charAt(0).toUpperCase()}
                                />
                            }
                            onClose={
                                isTeacher
                                    ? async () => {
                                          await removeStudentUC.execute(
                                              item._id!,
                                              sid
                                          )
                                          await refresh()
    ...
    return (
        ...
        >
                ...
                {isTeacher && (
                    <Button
                        ...
                        onPress={() => {
                            setEditing(null)
                            setName(
                                `[${category.name}] Grupo ${groups.length + 1}`
                            )
                            setShowDialog(true)
                        }}
                    >
            ...
            <Portal>
                        ...
                        <Button
                            ...
                            onPress={async () => {
                                ...
                                if (editing && editing._id) {
                                    await updateGroupUC.execute({
                                        _id: editing._id,
                                        name
                                    })
                                } else {
                                    await createGroupUC.execute({
                ...
                <Dialog
                    visible={showSelectStudents}
                    onDismiss={() => setShowSelectStudents(false)}
                >
                    ...
                    <Dialog.Actions>
                        <Button onPress={() => setShowSelectStudents(false)}>
                            Cancelar
                        </Button>
                        <Button
                            mode="contained"
                            ...
                            onPress={async () => {
                                if (!editing) return
                                const assigns = selectedStudentIds.map((sid) =>
                                    addStudentUC.execute(editing._id!, sid)
                                )
